...
class PopulateLinksCommand extends ContainerAwareCommand {
		...
		foreach ($animeObj as $anime) {
			...
			foreach ($episodes as $episode) {
				...
				try {
					...
					$this->logCrawling($episode, $crawlerService);
				}
			}
		}
	...
	private function logCrawling(AnimesEpisodes $episode, CrawlerService $crawlerService) {
		...
		$crawlerSeasonSettings = $crawlerSettings ? $crawlerSettings->getMinimalSeasonSettings($episode->getSeason()->getSeason()) : null;
		...
		fputcsv($this->importLogHandler, [
			...
			$crawlerSeasonSettings ? $crawlerSeasonSettings->getReset() ? 'yes' : 'no' : 'n.a',
			$crawlerSeasonSettings ? $crawlerSeasonSettings->getHandicap() : 'n.a',
